...
namespace MsWebGame.Portal.Database.DAO
{
    public class GiftCodeDAO
    {
        ...
        public int ReceiveGiftCode(long UserID, string GiftCode,int ServiceID, out long balance,out long giftCodeId,out long GiftCodeAmount)
        {
            balance = 0;
            DBHelper db = null;
            try
            {
                ...
                if (response == 1)
                {
                    balance = ConvertUtil.ToLong(param[2].Value);
                    giftCodeId= ConvertUtil.ToLong(param[3].Value);
                    GiftCodeAmount = ConvertUtil.ToLong(param[5].Value); ;
                }else
                {
                    balance = 0;
                    giftCodeId = 0;
