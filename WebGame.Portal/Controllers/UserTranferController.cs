            ...
            try
            {
                ...
                if (userReceive.AccountType == 2)//đại lý
                {
                    if (lngAmount < 100000)
                    {
                        ...
                        {
                            ResponseCode = -1005,
                            Message = String.Format(ErrorMsg.MinAmountTranfer, "500.000"),
                        };
                    }
                ...
                if (userReceive.AccountType != 2)//người dùng thường
                {
                    ...
                    if (Response == 1)
                    {
                        if (!String.IsNullOrEmpty(account.PhoneSafeNo))
                        {
                            int outResponse;
                            long msgID;
                            string msg = String.Format("Bạn vừa chuyển {0} tới tài khoản {1} tại thời điểm {2}", lngAmount.LongToMoneyFormat(), userReceive.NickName, DateTime.Now);
                        ...
                        string msgTele = String.Format("Bạn vừa chuyển {0} tới tài khoản {1} tại thời điểm {2}, Số dư tài khoản : {3}", lngAmount.LongToMoneyFormat(), userReceive.NickName, DateTime.Now, Wallet.LongToMoneyFormat());

                        SendTelePushTelegramID(3, msgTele, 0, false, account.AccountName);
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(stk))
                    {
                        ...
                        if (bankbanresponse == 1)
                        {
                            ...
                            UserDAO.Instance.UserComplainCreate(accountId, 3, 1, "Số tài khoản ngân hàng trong bank bẩn", "Số tài khoản ngân hàng trong bank bẩn", true, 1, out res2);
                            NLogManager.LogMessage(string.Format("UserTransferToAgencyAdmin- Khóa User {0} có số tài khoản bank bẩn {1}", accountId, note));
                            return new
                            {
                                ResponseCode = -1005,
                                Message = ErrorMsg.AccountLock
                    ...
                    if (Response == 1)
                    {
                        ...
                        try
                        {
                            var strNotifty = String.Format("Bạn vừa nhận {0} từ nickname {1}. Số dư hiện tại là {2}", lngAmount.LongToMoneyFormat(), account.AccountName, walletRecive.LongToMoneyFormat());
