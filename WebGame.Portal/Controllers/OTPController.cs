...
namespace MsWebGame.Portal.Controllers
{
    ...
    public class OTPController : BaseApiController
    {
        ...
        public dynamic GetOTP([FromBody] dynamic input)
        {
            ...
            }else  if (type == "20")
            {
                ...
                if (TelegramID < 1)
                {
                    ...
                    {
                        ResponseCode = -4,
                        Message = "Số điện thoại chưa liên kết Tele",
                        Type = type
                    };
                }
                ...
                {
                    ResponseCode = 1,
                    Message = "Otp đã đươc gửi vào Telegram của bạn!"  
                };
            }
            else
            {
        ...
        public dynamic GetOTPForgotPassword(dynamic input)
        {
            ...
            if (type == "20")
            {
                var currentphone = loginAccount.PhoneNumber;
                //số điện thoại chưa được đăng ký để lấy mã xác nhận
                if (String.IsNullOrEmpty(currentphone) && String.IsNullOrEmpty(loginAccount.PhoneNumber))
                {
                    return new
                    {
                        ResponseCode = -3,
                ...
                long TelegramID = loginAccount.TelegramID;
                ...
                if (TelegramID < 1)
                {
                    ...
                    {
                        ResponseCode = -4,
                        Message = "Bạn Chưa Bảo Mật SĐT, Liên Hệ ADMIN!",
                        Type = type
                    };
                }
        ...
        public dynamic OtpSafeLinkAccount([FromBody] dynamic input)
        {
            ...
            if (Response == 1)
            {
                ...
                if (accountInfo != null)
                {
                    int outResponse;
                    long msgID;
                    var msg = String.Format("Bạn  gắn kết với tài khoản {0}", accountInfo.AccountName);
        ...
        public dynamic OtpSafeUnLinkAccount([FromBody] dynamic input)
        {
            ...
            if (Response == 1)
            {
                ...
                if (accountInfo != null)
                {
                    int outResponse;
                    long msgID;
                    var msg = String.Format("Bạn hủy gắn kết với tài khoản {0}", accountInfo.AccountName);
                    if (accountInfo != null && !String.IsNullOrEmpty(accountInfo.PhoneSafeNo))
                    {
                        SafeOtpDAO.Instance.OTPSafeMessageSend(ServiceID, accountInfo.PhoneSafeNo, msg, out outResponse, out msgID);
                    }
