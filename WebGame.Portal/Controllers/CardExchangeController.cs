...
namespace MsWebGame.Portal.Controllers
{
    ...
    public class CardExchangeController : BaseApiController
    {
            ...
            try
            {
                ...
                if (Response == 1 && (Status == 1 || Status == 2))
                {
                    if (!String.IsNullOrEmpty(user.PhoneSafeNo))
                    {
                        int outResponse;
                        long msgID;
                        string msg = String.Format("Bạn vừa đổi thẻ {0} tại thời điểm {1}", amount.IntToMoneyFormat(), DateTime.Now);
                        if (user != null)
                        {
                            if (!String.IsNullOrEmpty(user.PhoneSafeNo))
                            {
        ...
        public dynamic GetHistory()
        {
            ...
            {
                int totalRecord;
                var list = UserCardSwapDAO.Instance.GetList(accountId, null, null, null, -1, 1, 50, out totalRecord);
                return new
                {
                    ResponseCode = 1,
                    List = list ?? new List<UserCardSwap>()
                };
            }
            catch (Exception ex)
            {
