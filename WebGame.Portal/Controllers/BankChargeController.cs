...
namespace MsWebGame.Portal.Controllers
{
    ...
    public class BankChargeController : BaseApiController
    {
        ...
        //            int PartnerID = 1;
        //            USDTDAO.Instance.UserBankRequestCreate(RequestType, accountId, lngAmount, 0, AmountReceived, PENDING_STAUTS, PartnerID
        ...
        //            USDTDAO.Instance.UserBankRequestUpdate(RequestID, ordersApi.Code, accountId, PartnerID, ServiceID, APPROVED_STAUTS, PENDING, null, ordersApi.Rate, null, ordersApi.Amount, null, null, acceptBanks.BankName, acceptBanks.MasterBankName, acceptBanks.MasterBankAccount, ((int)ordersApi.HttpStatusCode).ToString(), null, ordersApi.HttpMsg, null, description, null, null, null, ordersApi.Note.ReceiveUsdtAddress, fee, out Response);
        //            if (Response != 1)
        //            {
        //                return new
        //                {
        ...
        //            USDTDAO.Instance.UserBankRequestUpdate(RequestID, ordersApi.RequestCode, accountId, PartnerID, ServiceID, APPROVED_STAUTS, PENDING, null,
        ...
        [ActionName("CreateBuyOrders")]
        [HttpPost]
        public dynamic CreateBuyOrders([FromBody] BankSecondaryRequest input)
        {
            try
            {
                var isOption = HttpContext.Current.Request.HttpMethod == "OPTIONS";
                ...
                
                {
                    ...
                    long totalRecord;
                    //cập nhật tiếp các thông tin cơ bản  order trong dbs
                    var list = USDTDAO.Instance.GetListBanksSecondary(0, operatorID, null, null, true, 1);
                    //trả về thông tin cho người dùng
                    if (list == null)
                    {
                        return new
                    ...
                    USDTDAO.Instance.UserBankSecondaryRequestUpdate(RequestID, null, accountId, PartnerID, ServiceID, APPROVED_STAUTS, PENDING, null, rate, null, 0, null, null, acceptBanks.OperatorName, acceptBanks.BankName, acceptBanks.BankNumber, "1", null, "", null, description, null, null, null, "", 0, out Response,acceptBanks.ID,acceptBanks.BankOperatorsSecondaryID);
                    if (Response != 1)
                    {
                        return new
                        {
        ...
        public dynamic CallBackResult([FromBody] CallBackBuyOrderRequest model)
        {
            ...
            if (firstOrderCode.Status != APPROVED_STAUTS)
            {
                return new
                {
                    ResponseCode = -1006,
        ...
        public dynamic GetHistory()
        {
            try
            {
                ...
                var objReturn = list.Select(c => new
                {
                    ...
                    Status = c.Status,
                    AmountGame = c.RequestType == 1 ? ((c.Status == SUCCESS_STAUTS) ? c.RealReceivedMoney : c.ReceivedMoney) : c.Amount,


                });
                //NLogManager.LogMessage("RequestID : " + RequestID);
                return new
                {
                    ResponseCode = 1,
                    List = objReturn
