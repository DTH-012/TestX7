...
using Game.Accounts.Database.DAO;
    ...
    public class AccountController : BaseApiController
    {
            ...
            try
            {
                ...
                switch (loginType)
                {
                    case (int)Constants.enmAuthenType.AUTHEN_ID://login từ form =1
                        ...
                        //int IpResponse = 0;
                        //AccountDAO.Instance.IPCheck(deviceId, out IpResponse);
                        //if (IpResponse != 1)
                        //{
                        //    return new
                        //    {
                        ...
                        else if (response == 1 && registerAccount != null)
                        {
                            ...
                            //cộng tiền khi đăng ký mới

                            long transId = 0;
                            long wallet = 0;
                            var ReceiveMoney = 100000;
                            // cộng tiền user

                            //int resmoney = AdminDAO.Instance.AdminTransferToUser2(1, registerAccount.AccountID, (long)ReceiveMoney, "Thưởng đăng ký tài khoản số tiền : " + ReceiveMoney + "", 1, out transId, out wallet);
            ...
            try
            {
                ...
                if (accountSafeInfo.responseStatus == 1)
                {
                    ...
                    if (!String.IsNullOrEmpty(acountInfor.PhoneSafeNo))
                    {
                        int outResponse;
                        long msgID;
                        string msg = String.Format("Bạn vừa thực hiện giao dịch đóng băng {0} tại thời điểm {1}", amount.LongToMoneyFormat(), DateTime.Now);
            ...
            try
            {
                ...
                if (accountSafeInfo.responseStatus == 1)
                {
                    ...
                    if (!String.IsNullOrEmpty(user.PhoneSafeNo))
                    {
                        int outResponse;
                        long msgID;
                        string msg = String.Format("Bạn vừa thực hiện giao dịch rút két {0} tại thời điểm {1}", amount.LongToMoneyFormat(), DateTime.Now);
        ...
        public dynamic GiftCode([FromBody] dynamic input)
        {
            try
            {
                ...
                long giftCodeID;
                long GiftCodeAmount;
                int response = GiftCodeDAO.Instance.ReceiveGiftCode(accountId, GiftCode, ServiceID, out balance, out giftCodeID, out GiftCodeAmount);
                NLogManager.LogMessage(String.Format("GiftCode -{0}-AccountID{1}-Content:{2}", response,accountId,GiftCode.ToUpper()));
                if (response == 1)
                {
                    // int outResponse = 0;
                     String Description = String.Format("Tài khoản {2} Nhập mã gift code {0} với số tiền là {1}", GiftCode, GiftCodeAmount, displayName);
        ...
        public dynamic ForgotPassword(dynamic input)
        {
            ...
            string newPhone = phoneNumber.PhoneFormat();
            ...
            if (loginAccount != null)
            {
                ...
                if (response == 1)
                {
                    if (!String.IsNullOrEmpty(loginAccount.PhoneSafeNo))
                    {
                        int outResponse;
                        long msgID;
                        string msg = String.Format("Bạn vừa thay đổi mật khẩu  cho tài khoản {0} tại thời điểm {1}", loginAccount.AccountName, DateTime.Now);
