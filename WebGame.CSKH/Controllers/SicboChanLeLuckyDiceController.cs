...
{
    [AllowedIP]
    public class SicboChanLeLuckyDiceController : BaseController
    {
        // GET: Md5LuckyDice
        ...
        {
            //EventModel model = new EventModel();
            EventModel eventModel = SicboChanLeLuckyDiceDAO.Instance.GetEventInfo(model.EventID);
        ...
        public ActionResult GetEventList(GridCommand command)
        {
            ...
            var list = SicboChanLeLuckyDiceDAO.Instance.GetEventList();
        ...
        {
            int response = SicboChanLeLuckyDiceDAO.Instance.SaveEvent(model);
            if (response < 0)
            {
        ...
        {
            int response = SicboChanLeLuckyDiceDAO.Instance.DeleteEvent(model);
            if (response < 0)
            {
        ...
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        public ActionResult QuySicboChanLe()
        {
            ViewBag.Logs = SicboChanLeLuckyDiceDAO.Instance.GetSoiCau(15);
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [GridAction(EnableCustomBinding = true)]
        ...
        {
            //EventModel model = new EventModel();
            RaceTopModel eventModel = SicboChanLeLuckyDiceDAO.Instance.GetRaceTopInfo(model.RaceTopID);
        ...
        {
            int response = SicboChanLeLuckyDiceDAO.Instance.SaveRaceTop(model);
            if (response < 0)
            {
                string msg = MessageConvetor.MsgLuckyDice.GetEventMessage(response);
                ErrorNotification(msg);
        ...
        {
            int response = SicboChanLeLuckyDiceDAO.Instance.DeleteRaceTop(model);
            if (response < 0)
            {
                ErrorNotification(Message.DeleteFail);
            }
        ...
        {
            //lay danh sách chăm sóc khách hàng
            var list = SicboChanLeLuckyDiceDAO.Instance.GetRaceTopList();
        ...
        {
            int totalRecord = 0;
            var list = SicboChanLeLuckyDiceDAO.Instance.GetEventAwardList(model, command.Page - 1 <= 0 ? 1 : command.Page, AppConstants.CONFIG.GRID_SIZE, out totalRecord);
        ...
        private List<EventModel> ComboboxEvents()
        {
            ...
            var eventList = SicboChanLeLuckyDiceDAO.Instance.GetEventList();
        ...
        private List<RaceTopModel> ComboboxAwards()
        {
            ...
            var eventList = SicboChanLeLuckyDiceDAO.Instance.GetRaceTopList();
            list.AddRange(eventList);
        ...
        public ActionResult InitSession(GridCommand command)
        {
            ...
            SicboChanLeRedisSession result = _cachePvd.Get<SicboChanLeRedisSession>(keySession);
            if (result == null)
            {
                result = new SicboChanLeRedisSession();
            }
            long TotalBetTai = 0, TotalBetXiu = 0, TotalTai = 0, TotalXiu = 0;
            string key = Helpers.SicboChanLeLuckyDice.SicboChanLeHelper.GenerateKey(result.SessionID, (int)Helpers.SicboChanLeLuckyDice.KeyType.TotalBet, 0, (int)Helpers.SicboChanLeLuckyDice.BetSide.Tai);

            if (_cachePvd.Exists(key))
                TotalBetTai = _cachePvd.Get<long>(key);
            key = Helpers.SicboChanLeLuckyDice.SicboChanLeHelper.GenerateKey(result.SessionID, (int)Helpers.SicboChanLeLuckyDice.KeyType.TotalBet, 0, (int)Helpers.SicboChanLeLuckyDice.BetSide.Xiu);
            if (_cachePvd.Exists(key))
                TotalBetXiu = _cachePvd.Get<long>(key);
            key = Helpers.SicboChanLeLuckyDice.SicboChanLeHelper.GenerateKey(result.SessionID, (int)Helpers.SicboChanLeLuckyDice.KeyType.Turn, 0, (int)Helpers.SicboChanLeLuckyDice.BetSide.Tai);
            if (_cachePvd.Exists(key))
                TotalTai = _cachePvd.Get<long>(key);
            key = Helpers.SicboChanLeLuckyDice.SicboChanLeHelper.GenerateKey(result.SessionID, (int)Helpers.SicboChanLeLuckyDice.KeyType.Turn, 0, (int)Helpers.SicboChanLeLuckyDice.BetSide.Xiu);
            if (_cachePvd.Exists(key))
            ...
            string keyHu = CachingHandler.Instance.GeneralRedisKey("Sicbo", (result.CurrentState == 0 ? "UserBetBalance." : "UserBetBalance_Tmp.") + 2);
            ...
            keyHu = CachingHandler.Instance.GeneralRedisKey("Sicbo", (result.CurrentState == 0 ? "UserBetBalance." : "UserBetBalance_Tmp.") + 3);

            if (_cachePvd.Exists(keyHu))
            {
                BetBalancesXiu = _cachePvd.Get<List<UserBetBalance>>(keyHu);
            }
            keyHu = CachingHandler.Instance.GeneralRedisKey("Sicbo", "Mode2");
            ...
            return new JsonResult
            {
                ...
                Data = new
                {
                    ...
                    Logs = result.CurrentState == MsWebGame.CSKH.Helpers.SicboChanLeLuckyDice.GameState.ShowResult ? SicboChanLeLuckyDiceDAO.Instance.GetSoiCau(15) : new List<SoiCau>()
                },
        ...
        [AdminAuthorize(Roles = ADMIN_MONITOR_ROLE)]
        [HttpPost]
        public ActionResult SetModel(SicboChanLeLuckyDiceModel input)
        {
            //lay danh sách chăm sóc khách hàng
            string keyHu = CachingHandler.Instance.GeneralRedisKey("Sicbo", "Mode2");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            ...
            SicboChanLeRedisSession result = _cachePvd2.Get<SicboChanLeRedisSession>(keySession);
            if (result == null)
            {
                result = new SicboChanLeRedisSession();
            }
