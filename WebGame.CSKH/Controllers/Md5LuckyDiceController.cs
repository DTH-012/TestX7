...
{
    [AllowedIP]
    public class Md5LuckyDiceController : BaseController
    {
        // GET: Md5LuckyDice
        ...
        {
            //EventModel model = new EventModel();
            EventModel eventModel = Md5LuckyDiceDAO.Instance.GetEventInfo(model.EventID);
        ...
        public ActionResult GetEventList(GridCommand command)
        {
            ...
            var list = Md5LuckyDiceDAO.Instance.GetEventList();
        ...
        {
            int response = Md5LuckyDiceDAO.Instance.SaveEvent(model);
            if (response < 0)
            {
        ...
        {
            int response = Md5LuckyDiceDAO.Instance.DeleteEvent(model);
            if (response < 0)
            {
        ...
        {
            ViewBag.Logs = Md5LuckyDiceDAO.Instance.GetSoiCau(15);
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [GridAction(EnableCustomBinding = true)]
        ...
        {
            //EventModel model = new EventModel();
            RaceTopModel eventModel = Md5LuckyDiceDAO.Instance.GetRaceTopInfo(model.RaceTopID);
        ...
        {
            int response = Md5LuckyDiceDAO.Instance.SaveRaceTop(model);
            if (response < 0)
            {
                string msg = MessageConvetor.MsgLuckyDice.GetEventMessage(response);
                ErrorNotification(msg);
        ...
        {
            int response = Md5LuckyDiceDAO.Instance.DeleteRaceTop(model);
            if (response < 0)
            {
                ErrorNotification(Message.DeleteFail);
            }
        ...
        {
            //lay danh sách chăm sóc khách hàng
            var list = Md5LuckyDiceDAO.Instance.GetRaceTopList();
        ...
        {
            int totalRecord = 0;
            var list = Md5LuckyDiceDAO.Instance.GetEventAwardList(model, command.Page - 1 <= 0 ? 1 : command.Page, AppConstants.CONFIG.GRID_SIZE, out totalRecord);
        ...
        private List<EventModel> ComboboxEvents()
        {
            ...
            var eventList = Md5LuckyDiceDAO.Instance.GetEventList();
        ...
        private List<RaceTopModel> ComboboxAwards()
        {
            ...
            var eventList = Md5LuckyDiceDAO.Instance.GetRaceTopList();
            list.AddRange(eventList);
        ...
        {
            //lay danh sách chăm sóc khách hàng
            string keySession = "txmd5.session:info";

            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            Md5RedisSession result = _cachePvd.Get<Md5RedisSession>(keySession);
            if (result == null)
            {
                result = new Md5RedisSession();
            }
            long TotalBetTai = 0, TotalBetXiu = 0, TotalTai = 0, TotalXiu = 0;
            string key = Helpers.Md5LuckyDice.Md5Helper.GenerateKey(result.SessionID, (int)Helpers.Md5LuckyDice.KeyType.TotalBet, 0, (int)Helpers.Md5LuckyDice.BetSide.Tai);

            if (_cachePvd.Exists(key))
                TotalBetTai = _cachePvd.Get<long>(key);
            key = Helpers.Md5LuckyDice.Md5Helper.GenerateKey(result.SessionID, (int)Helpers.Md5LuckyDice.KeyType.TotalBet, 0, (int)Helpers.Md5LuckyDice.BetSide.Xiu);
            if (_cachePvd.Exists(key))
                TotalBetXiu = _cachePvd.Get<long>(key);
            key = Helpers.Md5LuckyDice.Md5Helper.GenerateKey(result.SessionID, (int)Helpers.Md5LuckyDice.KeyType.Turn, 0, (int)Helpers.Md5LuckyDice.BetSide.Tai);
            if (_cachePvd.Exists(key))
                TotalTai = _cachePvd.Get<long>(key);
            key = Helpers.Md5LuckyDice.Md5Helper.GenerateKey(result.SessionID, (int)Helpers.Md5LuckyDice.KeyType.Turn, 0, (int)Helpers.Md5LuckyDice.BetSide.Xiu);
            if (_cachePvd.Exists(key))
            ...
            string keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuMD5", (result.CurrentState == 0 ? "UserBetBalance." : "UserBetBalance_Tmp.") + 0);
            ...
            keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuMD5", (result.CurrentState == 0 ? "UserBetBalance." : "UserBetBalance_Tmp.") + 1);

            if (_cachePvd.Exists(keyHu))
            {
                BetBalancesXiu = _cachePvd.Get<List<UserBetBalance>>(keyHu);
            }
            keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuMD5", "Mode");
            ...
            foreach (var val in ListBetBalancesTai)
            {
                ...
                {
                    string _keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuMD5", "GetAccountGameProfit:" + result.SessionID + ":" + val.AccountID);
                    ...
                    string _keyHuHistoryInOut = CachingHandler.Instance.GeneralRedisKey("TaiXiuMD5", "HistoryInOut:" + result.SessionID + ":" + val.AccountID);
            ...
            return new JsonResult
            {
                ...
                Data = new
                {
                    ...
                    Logs = result.CurrentState == MsWebGame.CSKH.Helpers.Md5LuckyDice.GameState.ShowResult ? Md5LuckyDiceDAO.Instance.GetSoiCau(15) : new List<SoiCau>()
                },
        ...
        [AdminAuthorize(Roles = ADMIN_MONITOR_ROLE)]
        [HttpPost]
        public ActionResult SetModel(Md5LuckyDiceModel input)
        {
            //lay danh sách chăm sóc khách hàng
            string keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuMD5", "Mode");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            ...
            Md5RedisSession result = _cachePvd2.Get<Md5RedisSession>(keySession);
            if (result == null)
            {
                result = new Md5RedisSession();
            }
