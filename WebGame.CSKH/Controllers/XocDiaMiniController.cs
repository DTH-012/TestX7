...
using MsWebGame.CSKH.Models.XocDiaMini;
...
{
    [AllowedIP]
    public class XocDiaMiniController : BaseController
    {
        // GET: LuckyDice
        ...
        {
            //EventModel model = new EventModel();
            EventModel eventModel = XocDiaMiniDAO.Instance.GetEventInfo(model.EventID);
        ...
        public ActionResult GetEventList(GridCommand command)
        {
            ...
            var list = XocDiaMiniDAO.Instance.GetEventList();
        ...
        {
            int response = XocDiaMiniDAO.Instance.SaveEvent(model);
            if (response < 0)
            {
            ...
            return RedirectToAction("Event", "XocDiaMini");
        }
        ...
        {
            int response = XocDiaMiniDAO.Instance.DeleteEvent(model);
            if (response < 0)
            {
        ...
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        public ActionResult QuyXocDiaMini()
        {
            ViewBag.Logs = XocDiaMiniDAO.Instance.GetSoiCau(15);
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [GridAction(EnableCustomBinding = true)]
        ...
        {
            //EventModel model = new EventModel();
            RaceTopModel eventModel = XocDiaMiniDAO.Instance.GetRaceTopInfo(model.RaceTopID);
        ...
        {
            int response = XocDiaMiniDAO.Instance.SaveRaceTop(model);
            if (response < 0)
            {
                string msg = MessageConvetor.MsgLuckyDice.GetEventMessage(response);
                ErrorNotification(msg);
            ...
            return RedirectToAction("RaceTop", "XocDiaMini");
        }
        ...
        {
            int response = XocDiaMiniDAO.Instance.DeleteRaceTop(model);
            if (response < 0)
            {
                ErrorNotification(Message.DeleteFail);
            }
        ...
        {
            //lay danh sách chăm sóc khách hàng
            var list = XocDiaMiniDAO.Instance.GetRaceTopList();
        ...
        {
            int totalRecord = 0;
            var list = XocDiaMiniDAO.Instance.GetEventAwardList(model, command.Page - 1 <= 0 ? 1 : command.Page, AppConstants.CONFIG.GRID_SIZE, out totalRecord);
        ...
        private List<EventModel> ComboboxEvents()
        {
            ...
            var eventList = XocDiaMiniDAO.Instance.GetEventList();
        ...
        private List<RaceTopModel> ComboboxAwards()
        {
            ...
            var eventList = XocDiaMiniDAO.Instance.GetRaceTopList();
            list.AddRange(eventList);
        ...
        {
            //lay danh sách chăm sóc khách hàng
            string keySession = "xocdiamini.session:info";

            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            XocDiaMiniRedisSession result = _cachePvd.Get<XocDiaMiniRedisSession>(keySession);
            if (result == null)
            {
                result = new XocDiaMiniRedisSession();
            }

            List<UserBetBalance> BetBalancesTai = new List<UserBetBalance>();
            string keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini", "UserBetBalance." + 4);
            ...
            keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini", "UserBetBalance." + 1);

            if (_cachePvd.Exists(keyHu))
            {
                BetBalancesXiu = _cachePvd.Get<List<UserBetBalance>>(keyHu);
            }
            keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini", "Mode");
        ...
        [AdminAuthorize(Roles = ADMIN_MONITOR_ROLE)]
        [HttpPost]
        public ActionResult SetModel(XocDiaMiniModel input)
        {
            //lay danh sách chăm sóc khách hàng
            string keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini", "Mode");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            ...
            XocDiaMiniRedisSession result = _cachePvd2.Get<XocDiaMiniRedisSession>(keySession);
            if (result == null)
            {
                result = new XocDiaMiniRedisSession();
            }
