...
using MsWebGame.CSKH.Models.XocDiaMini4;
...
{
    [AllowedIP]
    public class XocDiaMini4Controller : BaseController
    {
        // GET: LuckyDice
        ...
        {
            //EventModel model = new EventModel();
            EventModel eventModel = XocDiaMini4DAO.Instance.GetEventInfo(model.EventID);
        ...
        public ActionResult GetEventList(GridCommand command)
        {
            ...
            var list = XocDiaMini4DAO.Instance.GetEventList();
        ...
        {
            int response = XocDiaMini4DAO.Instance.SaveEvent(model);
            if (response < 0)
            {
            ...
            return RedirectToAction("Event", "XocDiaMini4");
        }
        ...
        {
            int response = XocDiaMini4DAO.Instance.DeleteEvent(model);
            if (response < 0)
            {
        ...
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        public ActionResult QuyXocDiaMini4()
        {
            ViewBag.Logs = XocDiaMini4DAO.Instance.GetSoiCau(15);
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [GridAction(EnableCustomBinding = true)]
        ...
        {
            //EventModel model = new EventModel();
            RaceTopModel eventModel = XocDiaMini4DAO.Instance.GetRaceTopInfo(model.RaceTopID);
        ...
        {
            int response = XocDiaMini4DAO.Instance.SaveRaceTop(model);
            if (response < 0)
            {
                string msg = MessageConvetor.MsgLuckyDice.GetEventMessage(response);
                ErrorNotification(msg);
            ...
            return RedirectToAction("RaceTop", "XocDiaMini4");
        }
        ...
        {
            int response = XocDiaMini4DAO.Instance.DeleteRaceTop(model);
            if (response < 0)
            {
                ErrorNotification(Message.DeleteFail);
            }
        ...
        {
            //lay danh sách chăm sóc khách hàng
            var list = XocDiaMini4DAO.Instance.GetRaceTopList();
        ...
        {
            int totalRecord = 0;
            var list = XocDiaMini4DAO.Instance.GetEventAwardList(model, command.Page - 1 <= 0 ? 1 : command.Page, AppConstants.CONFIG.GRID_SIZE, out totalRecord);
        ...
        private List<EventModel> ComboboxEvents()
        {
            ...
            var eventList = XocDiaMini4DAO.Instance.GetEventList();
        ...
        private List<RaceTopModel> ComboboxAwards()
        {
            ...
            var eventList = XocDiaMini4DAO.Instance.GetRaceTopList();
            list.AddRange(eventList);
        ...
        {
            //lay danh sách chăm sóc khách hàng
            string keySession = "xocdiamini4.session:info";

            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            XocDiaMini4RedisSession result = _cachePvd.Get<XocDiaMini4RedisSession>(keySession);
            if (result == null)
            {
                result = new XocDiaMini4RedisSession();
            }

            List<UserBetBalance> BetBalancesTai = new List<UserBetBalance>();
            string keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini4", "UserBetBalance." + 4);
            ...
            keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini4", "UserBetBalance." + 1);

            if (_cachePvd.Exists(keyHu))
            {
                BetBalancesXiu = _cachePvd.Get<List<UserBetBalance>>(keyHu);
            }
            keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini4", "Mode");
        ...
        [AdminAuthorize(Roles = ADMIN_MONITOR_ROLE)]
        [HttpPost]
        public ActionResult SetModel(XocDiaMini4Model input)
        {
            //lay danh sách chăm sóc khách hàng
            string keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini4", "Mode");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            ...
            XocDiaMini4RedisSession result = _cachePvd2.Get<XocDiaMini4RedisSession>(keySession);
            if (result == null)
            {
                result = new XocDiaMini4RedisSession();
            }
