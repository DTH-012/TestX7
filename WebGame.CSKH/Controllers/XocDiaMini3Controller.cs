...
using MsWebGame.CSKH.Models.XocDiaMini3;
...
{
    [AllowedIP]
    public class XocDiaMini3Controller : BaseController
    {
        // GET: LuckyDice
        ...
        {
            //EventModel model = new EventModel();
            EventModel eventModel = XocDiaMini3DAO.Instance.GetEventInfo(model.EventID);
        ...
        public ActionResult GetEventList(GridCommand command)
        {
            ...
            var list = XocDiaMini3DAO.Instance.GetEventList();
        ...
        {
            int response = XocDiaMini3DAO.Instance.SaveEvent(model);
            if (response < 0)
            {
            ...
            return RedirectToAction("Event", "XocDiaMini3");
        }
        ...
        {
            int response = XocDiaMini3DAO.Instance.DeleteEvent(model);
            if (response < 0)
            {
        ...
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        public ActionResult QuyXocDiaMini3()
        {
            ViewBag.Logs = XocDiaMini3DAO.Instance.GetSoiCau(15);
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [GridAction(EnableCustomBinding = true)]
        ...
        {
            //EventModel model = new EventModel();
            RaceTopModel eventModel = XocDiaMini3DAO.Instance.GetRaceTopInfo(model.RaceTopID);
        ...
        {
            int response = XocDiaMini3DAO.Instance.SaveRaceTop(model);
            if (response < 0)
            {
                string msg = MessageConvetor.MsgLuckyDice.GetEventMessage(response);
                ErrorNotification(msg);
            ...
            return RedirectToAction("RaceTop", "XocDiaMini3");
        }
        ...
        {
            int response = XocDiaMini3DAO.Instance.DeleteRaceTop(model);
            if (response < 0)
            {
                ErrorNotification(Message.DeleteFail);
            }
        ...
        {
            //lay danh sách chăm sóc khách hàng
            var list = XocDiaMini3DAO.Instance.GetRaceTopList();
        ...
        {
            int totalRecord = 0;
            var list = XocDiaMini3DAO.Instance.GetEventAwardList(model, command.Page - 1 <= 0 ? 1 : command.Page, AppConstants.CONFIG.GRID_SIZE, out totalRecord);
        ...
        private List<EventModel> ComboboxEvents()
        {
            ...
            var eventList = XocDiaMini3DAO.Instance.GetEventList();
        ...
        private List<RaceTopModel> ComboboxAwards()
        {
            ...
            var eventList = XocDiaMini3DAO.Instance.GetRaceTopList();
            list.AddRange(eventList);
        ...
        {
            //lay danh sách chăm sóc khách hàng
            string keySession = "xocdiamini3.session:info";

            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            XocDiaMini3RedisSession result = _cachePvd.Get<XocDiaMini3RedisSession>(keySession);
            if (result == null)
            {
                result = new XocDiaMini3RedisSession();
            }

            List<UserBetBalance> BetBalancesTai = new List<UserBetBalance>();
            string keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini3", "UserBetBalance." + 4);
            ...
            keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini3", "UserBetBalance." + 1);

            if (_cachePvd.Exists(keyHu))
            {
                BetBalancesXiu = _cachePvd.Get<List<UserBetBalance>>(keyHu);
            }
            keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini3", "Mode");
        ...
        [AdminAuthorize(Roles = ADMIN_MONITOR_ROLE)]
        [HttpPost]
        public ActionResult SetModel(XocDiaMini3Model input)
        {
            //lay danh sách chăm sóc khách hàng
            string keyHu = CachingHandler.Instance.GeneralRedisKey("XocDiaMini3", "Mode");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            ...
            XocDiaMini3RedisSession result = _cachePvd2.Get<XocDiaMini3RedisSession>(keySession);
            if (result == null)
            {
                result = new XocDiaMini3RedisSession();
            }
