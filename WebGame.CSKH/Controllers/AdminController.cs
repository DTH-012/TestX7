...
namespace MsWebGame.CSKH.Controllers
{
    ...
    public class AdminController : BaseController
    {
        ...
        public dynamic BanUserChat(string nickName, int rdoStatus, int serviceId)
        {
            ...
            NLogManager.LogMessage("adminName: " + adminName);
            bool res = false;
            if (rdoStatus == 0)
            {
                string uri = "api/chat/AddBanUser";
                var resLock = SendRequestApi.Instance.SendLockChat(adminName, nickName, uri, serviceId);
                res = resLock;
            }
            else if (rdoStatus == 1)
            {
                string uri = "api/chat/RemoveBanUser";
                var resUnlock = SendRequestApi.Instance.SendUnLockChat(adminName, nickName, uri, serviceId);
