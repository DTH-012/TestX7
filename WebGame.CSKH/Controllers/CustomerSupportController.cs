...
//using MsWebGame.CSKH.Models.Md5LuckyDice;
...
namespace MsWebGame.CSKH.Controllers
{
    ...
    public class CustomerSupportController : BaseController
    {
        ...
        public ActionResult GetHistoryPlay(GridCommand command, ParCustomerSupport data)
        {
            ...
            NLogManager.LogMessage("accInfo.AccountID:" + accInfo.AccountID);
            NLogManager.LogMessage("gameId:" + gameId);
            NLogManager.LogMessage("spinId:" + spinId);
            NLogManager.LogMessage("fromDate:" + fromDate);
            NLogManager.LogMessage("toDate:" + toDate);
            NLogManager.LogMessage("orderby:" + orderby);
            NLogManager.LogMessage("command.Page - 1 <= 0 ? 1 : command.Page:" + (command.Page - 1 <= 0 ? 1 : command.Page));
            NLogManager.LogMessage("totalrecord:" + totalrecord);
            if (lstRs == null)
        ...
        // Check lịch sử chơi tài xỉu Md5
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        public ActionResult HistoryPlayTaiXiuMd5()
        {
            ViewBag.GameBox = InfoHandler.Instance.GetGameBox();
            ViewBag.ServiceBox = GetServices();
            ViewBag.Orders = InfoHandler.Instance.GetOrderHistoryPlay();
        ...
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult GetHistoryPlayTaiXiuMd5(GridCommand command, ParCustomerSupport data)
        {
            long? spinId = data.SpinID;
            if (spinId == null)
            {
            ...
            int page = command.Page - 1 <= 0 ? 0 : command.Page - 1;
            int totalrecord = 0;
            var lstRs = CustomerSupportDAO.Instance.GetHistoryPlayMd5((long)spinId);
            if (lstRs == null)
                lstRs = new List<HistoryPlay>();
            totalrecord = lstRs.Count;
            lstRs = lstRs.Skip(page * 20).Take(20).ToList();
        ...
        // End Check lịch sử chơi tài xỉu Md5
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        ...
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        public ActionResult AccountOverviewPopup(string NickName = "")
        {
            ViewBag.ServiceBox = GetServices();
            ViewBag.Value = NickName;
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        [HttpPost]
        public ActionResult AccountOverviewPopup(ParCustomerSupport data)
        {
            ...
            //NLogManager.LogMessage("Prama: " + value);
            if (account == null)
                return View();
        ...
        {
            //NLogManager.LogMessage("GAME ID: " + gameId);
            if (gameId < 1)
                return null;
            ...
            //NLogManager.LogMessage("RESULT AccountGameProfit: " + json);
            return json;
        }
        ...
        public dynamic GetGameInfo()
        {
            try
            {
                ...
                //NLogManager.LogMessage("RESULT GAME INFO: " + json);
                return json;
            }
            catch (Exception ex)
            {
        ...
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        public ActionResult ActionJackpot()
        {
            ViewBag.GameBox = InfoHandler.Instance.GetGameBox();
            ViewBag.ServiceBox = GetServices();
            return View();
        ...
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult GetClickJackpot(GridCommand command, ParCustomerSupport data)
        {
            ...
            int totalrecord = 0;
            var lstRs = CustomerSupportDAO.Instance.GetHistoryJackpotSession(Status, AppConstants.CONFIG.GRID_SIZE, command.Page - 1 <= 0 ? 1 : command.Page,
                out totalrecord);
            if (lstRs == null)
                lstRs = new List<HistoryClickJackpot>();

            var model = new GridModel<HistoryClickJackpot> { Data = lstRs, Total = totalrecord };
            return new JsonResult { Data = model };
        }
        ...
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [HttpPost]
        public ActionResult PopupConfigJackpot(Models.Param.JackpotSetPar data)
        {
            ViewBag.Nickname = data.Nickname;
            ViewBag.Count = data.Count > 0 ? data.Count : 1;
            ViewBag.Id = data.Id;
            ViewBag.GamesJackpot = new List<KeyValuePair<int, string>>() {
                new KeyValuePair<int, string>(100, "Cowboy"),
                new KeyValuePair<int, string>(200, "Tam Quốc"),
                new KeyValuePair<int, string>(300, "Thuỷ Cung"),
                new KeyValuePair<int, string>(400, "Songoku"),
                new KeyValuePair<int, string>(500, "Ai Cập"),

                new KeyValuePair<int, string>(600, "Kim Cương"),
                new KeyValuePair<int, string>(700, "MiniPoker"),
                new KeyValuePair<int, string>(800, "Siêu xe"),

                new KeyValuePair<int, string>(900, "Panda"),
                new KeyValuePair<int, string>(111, "Zombie"),
                new KeyValuePair<int, string>(222, "Thần Sét"),
                new KeyValuePair<int, string>(333, "SuperHerDes"),
                new KeyValuePair<int, string>(444, "Thần Rừng"),
                new KeyValuePair<int, string>(555, "Gái Nhảy"),
                new KeyValuePair<int, string>(666, "Fozen"),
                new KeyValuePair<int, string>(777, "Super Mario"),
                new KeyValuePair<int, string>(888, "Pokemon"),


            };
            ViewBag.ServiceBox = GetServices();
            ViewBag.Bets = new List<int>() { 100, 1000, 10000, 2000, 5000, 30000 };
            return PartialView();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [HttpPost]
        public ActionResult PopupConfigJackpotSave(Models.Param.JackpotSetPar data)
        {

            int rs = CustomerSupportDAO.Instance.UpdateHistoryJackpotSession(data.Id, data.GameId, data.RoomId, data.Count, data.Nickname);
            return new JsonResult
            {
                Data = new
                {
        ...
        [AdminAuthorize(Roles = ADMIN_ALL_ROLE)]
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult GetListTyLeSlot(GridCommand command, ParCustomerSupport data)
        {
