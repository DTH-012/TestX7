...
namespace MsWebGame.CSKH.Controllers
{
    ...
    {
        private static readonly string _CARDCALLBACK = System.Configuration.ConfigurationManager.AppSettings["CARD_CALLBACK"].ToString();
        ...
        public ActionResult UserCardSwapExamine(long userCardSwapId, long userId, int checkStatus)
        {
            NLogManager.LogMessage(string.Format("AdminID:{5} | CardSwapId:{0} | UserId:{1} | Balance:{2} | Status: {3} | Response:{4}",
                       userCardSwapId, userId, 0, checkStatus, 0, AdminID));
            ...
            string CardNumberSend = string.Empty;
            string CardSerialSend = string.Empty;
            string Operation = string.Empty;
            string TransactionCode = DateTime.Now.Millisecond.ToString() + DateTime.Now.Second.ToString() + DateTime.Now.Minute.ToString() + DateTime.Now.Hour.ToString() + DateTime.Now.Day.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Year.ToString();

            switch (card.TelOperatorCode)
            {
                case "VTT":
                    Operation = "Viettel";
                    break;
            ...
            // call api trả về thẻ 
            int res = 0;
            if (checkStatus == 1) // nếu đồng ý hệ thống sẽ call api đổi thẻ
            {

                // gửi request : CARD_CALLBACK
                // get value ID 
               
                var sendModel = new BuyCardAvengeModel();
                sendModel.TransactionCode = TransactionCode;
                sendModel.HomeNetwork = Operation;
                sendModel.CardValue = (int)card.CardValue;
                var bodySend = Newtonsoft.Json.JsonConvert.SerializeObject(sendModel);
                var client = new RestClient(_CARDCALLBACK);
                client.Timeout = -1;
                var request = new RestRequest(Method.POST);
                request.AddHeader("Content-Type", "application/json");
                request.AddParameter("application/json", bodySend, ParameterType.RequestBody);
                IRestResponse response = client.Execute(request);
                // var str = "{'status':200,'message':{'Id':21}}"; fake data
                var objresult = JsonConvert.DeserializeObject<ResponseBuyCard>(response.Content);


                if (objresult.message.Id > 0) // gửi thành công
                {
                    // lấy dữ liệu callback
                    //Thread.Sleep(5); // dừng 5s chờ callback
                    //
                    Thread.Sleep(TimeSpan.FromSeconds(10));
                    var callback = CardDAO.Instance.UserCardSwapCallBack(TransactionCode);
                    if (callback == null) // nếu callback chưa xuất hiện data -> return luôn
                    {
                        return Json(new
                        {
                    ...
                    else // ngược lại sẽ lấy dữ liệu call back gửi cho khách hàng
                    {
                        CardNumberSend = callback.Code;
                        CardSerialSend = callback.Seri;
                    }
                    res = CardDAO.Instance.UserCardSwapExamine(AdminID, userCardSwapId, userId, checkStatus, out balance, CardNumberSend, CardSerialSend, card.CardValue);
                    NLogManager.LogMessage(string.Format("AdminID:{5} | CardSwapId:{0} | UserId:{1} | Balance:{2} | Status: {3} | Response:{4}",
            ...
            NLogManager.LogMessage(string.Format("log2 : {0}", res));
            if (res == 1)
            {
                return Json(new
                {
