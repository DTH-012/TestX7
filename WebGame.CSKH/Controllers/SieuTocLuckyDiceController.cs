...
using MsWebGame.CSKH.Models.SieuTocLuckyDice;
...
{
    [AllowedIP]
    public class SieuTocLuckyDiceController : BaseController
    {
        // GET: Md5LuckyDice
        ...
        {
            //EventModel model = new EventModel();
            EventModel eventModel = SieuTocLuckyDiceDAO.Instance.GetEventInfo(model.EventID);
        ...
        public ActionResult GetEventList(GridCommand command)
        {
            ...
            var list = SieuTocLuckyDiceDAO.Instance.GetEventList();
        ...
        {
            int response = SieuTocLuckyDiceDAO.Instance.SaveEvent(model);
            if (response < 0)
            {
        ...
        {
            int response = SieuTocLuckyDiceDAO.Instance.DeleteEvent(model);
            if (response < 0)
            {
        ...
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        public ActionResult QuySieuToc()
        {
            ViewBag.Logs = SieuTocLuckyDiceDAO.Instance.GetSoiCau(15);
            return View();
        }
        [AdminAuthorize(Roles = ADMIN_ROLE)]
        [GridAction(EnableCustomBinding = true)]
        ...
        {
            //EventModel model = new EventModel();
            RaceTopModel eventModel = SieuTocLuckyDiceDAO.Instance.GetRaceTopInfo(model.RaceTopID);
        ...
        {
            int response = SieuTocLuckyDiceDAO.Instance.SaveRaceTop(model);
            if (response < 0)
            {
                string msg = MessageConvetor.MsgLuckyDice.GetEventMessage(response);
                ErrorNotification(msg);
        ...
        {
            int response = SieuTocLuckyDiceDAO.Instance.DeleteRaceTop(model);
            if (response < 0)
            {
                ErrorNotification(Message.DeleteFail);
            }
        ...
        {
            //lay danh sách chăm sóc khách hàng
            var list = SieuTocLuckyDiceDAO.Instance.GetRaceTopList();
        ...
        {
            int totalRecord = 0;
            var list = SieuTocLuckyDiceDAO.Instance.GetEventAwardList(model, command.Page - 1 <= 0 ? 1 : command.Page, AppConstants.CONFIG.GRID_SIZE, out totalRecord);
        ...
        private List<EventModel> ComboboxEvents()
        {
            ...
            var eventList = SieuTocLuckyDiceDAO.Instance.GetEventList();
        ...
        private List<RaceTopModel> ComboboxAwards()
        {
            ...
            var eventList = SieuTocLuckyDiceDAO.Instance.GetRaceTopList();
            list.AddRange(eventList);
        ...
        {
            //lay danh sách chăm sóc khách hàng
            string keySession = "txsieutoc.session:info";

            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            SieuTocRedisSession result = _cachePvd.Get<SieuTocRedisSession>(keySession);
            if (result == null)
            {
                result = new SieuTocRedisSession();
            }
            long TotalBetTai = 0, TotalBetXiu = 0, TotalTai = 0, TotalXiu = 0;
            string key = Helpers.SieuTocLuckyDice.SieuTocHelper.GenerateKey(result.SessionID, (int)Helpers.SieuTocLuckyDice.KeyType.TotalBet, 0, (int)Helpers.SieuTocLuckyDice.BetSide.Tai);

            if (_cachePvd.Exists(key))
                TotalBetTai = _cachePvd.Get<long>(key);
            key = Helpers.SieuTocLuckyDice.SieuTocHelper.GenerateKey(result.SessionID, (int)Helpers.SieuTocLuckyDice.KeyType.TotalBet, 0, (int)Helpers.SieuTocLuckyDice.BetSide.Xiu);
            if (_cachePvd.Exists(key))
                TotalBetXiu = _cachePvd.Get<long>(key);
            key = Helpers.SieuTocLuckyDice.SieuTocHelper.GenerateKey(result.SessionID, (int)Helpers.SieuTocLuckyDice.KeyType.Turn, 0, (int)Helpers.SieuTocLuckyDice.BetSide.Tai);
            if (_cachePvd.Exists(key))
                TotalTai = _cachePvd.Get<long>(key);
            key = Helpers.SieuTocLuckyDice.SieuTocHelper.GenerateKey(result.SessionID, (int)Helpers.SieuTocLuckyDice.KeyType.Turn, 0, (int)Helpers.SieuTocLuckyDice.BetSide.Xiu);
            if (_cachePvd.Exists(key))
            ...
            string keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuSIEUTOC", (result.CurrentState == 0 ? "UserBetBalance." : "UserBetBalance_Tmp.") + 0);
            ...
            keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuSIEUTOC", (result.CurrentState == 0 ? "UserBetBalance." : "UserBetBalance_Tmp.") + 1);

            if (_cachePvd.Exists(keyHu))
            {
                BetBalancesXiu = _cachePvd.Get<List<UserBetBalance>>(keyHu);
            }
            keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuSIEUTOC", "Mode");
            ...
            foreach (var val in ListBetBalancesTai)
            {
                ...
                {
                    string _keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuSIEUTOC", "GetAccountGameProfit:" + result.SessionID + ":" + val.AccountID);
                    ...
                    string _keyHuHistoryInOut = CachingHandler.Instance.GeneralRedisKey("TaiXiuSIEUTOC", "HistoryInOut:" + result.SessionID + ":" + val.AccountID);
            ...
            return new JsonResult
            {
                ...
                Data = new
                {
                    ...
                    Logs = result.CurrentState == MsWebGame.CSKH.Helpers.SieuTocLuckyDice.GameState.ShowResult ? SieuTocLuckyDiceDAO.Instance.GetSoiCau(15) : new List<SoiCau>()
                },
        ...
        [AdminAuthorize(Roles = ADMIN_MONITOR_ROLE)]
        [HttpPost]
        public ActionResult SetModel(SieuTocLuckyDiceModel input)
        {
            //lay danh sách chăm sóc khách hàng
            string keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiuSIEUTOC", "Mode");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            ...
            SieuTocRedisSession result = _cachePvd2.Get<SieuTocRedisSession>(keySession);
            if (result == null)
            {
                result = new SieuTocRedisSession();
            }
