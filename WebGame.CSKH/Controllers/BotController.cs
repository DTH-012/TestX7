...
namespace MsWebGame.CSKH.Controllers
{
    ...
    public class BotController : BaseController
    {
        ...
        public ActionResult BotInjectMoney(BotFundInfo model)
        {
            ...
            if ((typeFund != 1 && typeFund != 2) || string.IsNullOrEmpty(amount))
            {
            ...
            if (iamount < 10000)
            {
                ViewBag.Message = "Số tiền tối thiểu là 10.000!";
                return View(model);
            }

            AddFundSets(iamount);
            //int res = BotDAO.Instance.AllInjectMoney(model.TypeFund, iamount);
            string msg = string.Format("bot {0}", typeFund == 1 ? "tài xỉu" : "slot");
        ...
        public BotFundSets AddFundSets(long money)
        {
            string keyHu = CachingHandler.Instance.GeneralRedisKey("TaiXiu", "FundSets");
            RedisCacheProvider _cachePvd = new RedisCacheProvider();
            if (_cachePvd.Exists(keyHu))
            {
                ...
                {
                    botFundSets.Balance += money;
                    botFundSets.TotalAddBalance += money;
                    _cachePvd.Set(keyHu, botFundSets);
                }
                return botFundSets;
            }
