...
namespace MsWebGame.CSKH.Database.DAO
{
    public class CustomerSupportDAO
    {
        ...
        public List<HistoryPlay> GetHistoryPlayMd5(long spinId)
        {
            DBHelper db = null;
            try
            {
                var pars = new SqlParameter[2];
                pars[0] = new SqlParameter("@_GameType", 68);
                pars[1] = new SqlParameter("@_SpinID", spinId);
                db = new DBHelper(Config.BettingLogConn);
                var lstRs = db.GetListSP<HistoryPlay>("SP_HistoryPlay_Search_TXMD5", pars);
                return lstRs;
            }
            catch (Exception ex)
            {
        ...
        {
            DBHelper db = null;
            //NLogManager.LogMessage("Search Type: " + type + " Giá Trị: " + value + " Service ID: " + serviceid);
            try
            {
                var pars = new SqlParameter[3];
                pars[0] = new SqlParameter("@_SearchType", type);
        ...
        public List<HistoryClickJackpot> GetHistoryJackpotSession(int Status, int maxRow, int CurrentPage, out int totalrecord)
        {
            DBHelper db = null;
            totalrecord = 0;
            List<HistoryClickJackpot> lstRs = new List<HistoryClickJackpot>();
            try
            {
                db = new DBHelper(Config.BettingConn);
                ...
                param.Add(new SqlParameter("@DATE_START", ""));
                param.Add(new SqlParameter("@DATE_END", ""));

                param.Add(new SqlParameter("@_TotalRecord", SqlDbType.Int) { Direction = ParameterDirection.Output });

                lstRs = db.GetListSP<HistoryClickJackpot>("SP_Jackpot_Set_Pagination", param.ToArray());
        ...
        public int UpdateHistoryJackpotSession(int id, int gameId, int roomId, int count, string nickname)
        {
            ...
            try
            {
                ...
                pars[2] = new SqlParameter("@_Count", count);
                pars[3] = new SqlParameter("@_GameID", gameId);
                pars[4] = new SqlParameter("@_RoomID", roomId);

                pars[5] = new SqlParameter("@_Response", SqlDbType.BigInt) { Direction = ParameterDirection.Output };

                db.ExecuteNonQuerySP("SP_Jackpot_UpdateSave", pars);
                int Response = ConvertUtil.ToInt(pars[5].Value);
        ...
        public long GifcodeNewFromUser(long accountId)
        {
            DBHelper db = null;
            try
            {
                ...
                db.ExecuteNonQuerySP("SP_CheckGifcodeNewFromUser", pars);
                return ConvertUtil.ToLong(pars[1].Value); ;
            }
            catch (Exception ex)
            {
