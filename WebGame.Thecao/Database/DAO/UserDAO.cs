...
namespace MsWebGame.Thecao.Database.DAO
{
    public class UserDAO
    {
        ...
        public GetDisPlayName GetDisPlayGame(long UserID)
        {
            DBHelper db = null;
            try
            {
                ...
                param[0] = new SqlParameter("@_UserID", SqlDbType.Int);
                param[0].Value = UserID;

                var _GetDisPlayName = db.GetInstanceSP<GetDisPlayName>("SP_Users_GetDisPlayName", param.ToArray());
        ...
        public string USER_RECHARGE_CHECK3(long? UserID)
        {
            DBHelper db = null;
            try
            {
                db = new DBHelper(Config.BettingConn);
                SqlConnection conn = new SqlConnection(Config.BettingConn);
                SqlCommand cmd = new SqlCommand("SELECT  [dbo].[fun_USERRECHANGE](" + UserID + ")", conn);
